-- ============================================================
-- STUDYLY — SCHEMA (PDF materials only)
...
create policy "Public read approved colleges" on public.colleges for select using (approved = true);
create policy "Anyone can submit college" on public.colleges for insert with check (approved = false);
...
create policy "Public read approved materials" on public.materials for select using (approved = true);
create policy "Anyone can upload" on public.materials for insert with check (true);

-- UPVOTES (prevent double voting using a browser-generated key)
...
-- Indexes
create index on public.materials (college_id, branch, year, approved, upvotes desc);
create index on public.materials (college_id, branch, year, subject, approved);
...
--   with check (bucket_id = 'materials' AND right(name, 4) = '.pdf');
--
